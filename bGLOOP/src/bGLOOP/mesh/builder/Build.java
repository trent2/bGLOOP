...
public class Build implements BuilderInterface {
	...
	public void addVertexGeometric(float x, float y, float z) {
    	if(vertexCoordinateRanges == null) {
    		vertexCoordinateRanges = new float[]  { x, x, y, y, z, z };  
    	} else {
    		if(x<vertexCoordinateRanges[0]) vertexCoordinateRanges[0] = x;
    		if(x>vertexCoordinateRanges[1]) vertexCoordinateRanges[1] = x;
    		if(y<vertexCoordinateRanges[2]) vertexCoordinateRanges[2] = y;
    		if(y>vertexCoordinateRanges[3]) vertexCoordinateRanges[3] = y;
    		if(z<vertexCoordinateRanges[4]) vertexCoordinateRanges[4] = z;
    		if(z>vertexCoordinateRanges[5]) vertexCoordinateRanges[5] = z;
    	}
...
//            log.log(INFO,"Adding vertex g=" + vertexIndices[loopi] + " t=" + vertexIndices[loopi + 1] + " n=" + vertexIndices[loopi + 2]);
            ...
            if (vertexIndex != EMPTY_VERTEX_VALUE) {
                ...
                if (vertexIndex - 1 >= 0 && vertexIndex - 1 < verticesN.size()) {
                	...
                	// it's normal that there's no normal
...
//    }
	...
	public void doneParsingObj(String filename) {
    	float mx = (vertexCoordinateRanges[0]+vertexCoordinateRanges[1])/2,
    		   my = (vertexCoordinateRanges[2]+vertexCoordinateRanges[3])/2,
    		   mz = (vertexCoordinateRanges[4]+vertexCoordinateRanges[5])/2;

    	// center mid to origin
    	for(VertexGeometric vg : verticesG) {
    		vg.x -= mx; vg.y -= my; vg.z -= mz;
    	}
